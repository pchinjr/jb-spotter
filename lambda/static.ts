...
function buildInternalUrl(basePath: string, targetPath: string): string {
...
function getRequestHost(event: APIGatewayProxyEvent): string | undefined {
  const headers = event.headers || {};
  return headers.Host || headers.host || event.requestContext?.domainName;
}

function isDefaultExecuteApiHost(host?: string): boolean {
  return !!host && /\.execute-api\.[^.]+\.amazonaws\.com$/i.test(host);
}

function getBasePath(event: APIGatewayProxyEvent): string {
