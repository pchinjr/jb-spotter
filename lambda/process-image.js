...
function calculateSmartPosition(faces, targetWidth, targetHeight, jeffWidth, jeffHeight) {
    ...
    const scores = candidates.map(pos => {
        ...
        if (!hasOverlap) {
            ...
            for (const face of faceRegions) {
                ...
                const distance = Math.sqrt(Math.pow(faceCenter.x - jeffCenter.x, 2) +
...
async function getJeffBarrImage() {
    try {
...
async function compositeImages(userImageBuffer, jeffBarrBuffer, metadata, faces) {
    // First, auto-rotate the image based on EXIF to get correct orientation
    const rotatedImage = await (0, sharp_1.default)(userImageBuffer)
        .rotate() // Auto-rotate based on EXIF orientation
        .toBuffer();
    // Get metadata AFTER rotation to get the correct dimensions
    const rotatedMetadata = await (0, sharp_1.default)(rotatedImage).metadata();
    ...
    const resizedUserImage = await (0, sharp_1.default)(rotatedImage)
    ...
    const resizedMetadata = await (0, sharp_1.default)(resizedUserImage).metadata();
    ...
    const jeffMetadata = await (0, sharp_1.default)(jeffBarrBuffer).metadata();
    ...
    const jeffScale = Math.min((finalWidth * 0.45) / originalJeffWidth, (finalHeight * 0.65) / originalJeffHeight);
    const jeffWidth = Math.round(originalJeffWidth * jeffScale);
    const jeffHeight = Math.round(originalJeffHeight * jeffScale);
    // Resize Jeff's image
    const resizedJeffBuffer = await (0, sharp_1.default)(jeffBarrBuffer)
        .resize(jeffWidth, jeffHeight, {
        fit: 'inside'
    })
        .toBuffer();
    // Calculate smart positioning based on detected faces
    const position = calculateSmartPosition(faces, finalWidth, finalHeight, jeffWidth, jeffHeight);
    ...
    try {
        return await (0, sharp_1.default)(resizedUserImage)
            .composite([{
...
async function uploadProcessedImage(imageBuffer) {
